...
require_once '../models/TMDB/TMDBRatedTvShow.php';
...
if (isset($_SESSION['signed_in']) && $_SESSION['signed_in'] === true) {
    ...
    if (!isset($_SESSION['user_platform_ids']['tmdb'])) {
        ...
        if (!$account_data || !isset($account_data['id'])) {
            ...
            $url = TMDB_API_URL . "/account/{$account_id}/rated/tv?api_key=" . TMDB_API_KEY . "&session_id=" . $session_id;
            ...
            if (curl_errno($ch)) {
                ...
                if (isset($data['results'])) {
                    foreach ($data['results'] as $tvJson) {
                        $ratedTvShows[] = new TMDBRatedTvShow($tvJson);
                    }
                } else {
                    $error = "No rated TV shows found.";
                }
            }
